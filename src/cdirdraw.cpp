...
BOOL CDirDraw::InitDirectDraw() {
	...
	HRESULT result = m_pDirectDrawObj->SetCooperativeLevel(m_hWindow,
...
BOOL CDirDraw::CreateMainSurfaces() {
	...
	HRESULT result = m_pDirectDrawObj->CreateSurface(&ddSurfaceDesc, &m_pPrimarySurface, NULL);
	...
	result = m_pPrimarySurface->GetAttachedSurface(&ddsCaps, &m_pBackBuffer);
...
BOOL CDirDraw::SetTransparentColors(UINT surfaceNum,
	...
	HRESULT result = pSurface->SetColorKey(DDCKEY_SRCBLT, &ddColorKey);
